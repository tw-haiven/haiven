...
test.describe('Cards Exploration Tests', () => {
  ...
  test('should test basic cards functionality on STRIDE page', async ({ page }) => {
    ...
    await page.goto('http://localhost:8080/boba/cards?prompt=threat-modelling-cdbaba6f');
    await page.waitForLoadState('networkidle');
  ...
  test('should test cards functionality on Requirements Breakdown page', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      return hasCards || hasSubstantialContent;
    }, { timeout: 30000 });
    
    // Verify cards are displayed - use multiple selectors to be more robust
    const cards = await page.locator('.ant-card, [data-testid="card"], .card').count();
    expect(cards).toBeGreaterThan(0);
  });
  ...
  test('should test cards functionality on Fitness Functions page', async ({ page }) => {
    ...
    await page.goto('http://localhost:8080/boba/cards?prompt=fitness-functions-823199aa');
    await page.waitForLoadState('networkidle');
