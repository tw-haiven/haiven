...
const Home = () => {
  ...
  const onSubmitPrompt = async (value, event) => {
    ...
    const uri = '/api/make-scenario' 
          ...
          +'&time_horizon=' + encodeURIComponent(timeHorizon)
          +'&optimism=' + encodeURIComponent(optimism)
          +'&realism=' + encodeURIComponent(realism)
          +'&strangeness=' + encodeURIComponent(strangeness)
        ...
        <div id="prompt-center">
          ...
          <div style={{marginTop: 10}}>
            ...
            <Select defaultValue={'optimistic'} onChange={handleSelectOptimismChange} style={{ width: 150 }} disabled={isLoading}
              options={[
                { value: 'optimistic', label: <div><span className='config-icon'><AiOutlineLike /></span> Optimistic</div> },
            ...
            <Select defaultValue={'futuristic sci-fi'} onChange={handleSelectRealismChange} style={{ width: 150 }} disabled={isLoading}
              options={[
                ...
                { value: 'futuristic sci-fi', label: <div><span className='config-icon'><AiOutlineRocket/></span> Sci-fi Future</div> },
          ...
          {scenarios.map((scenario, i) => {
            return <Card key={i} className="scenario" title={
              ...
              <div className="scenario-card-content">
                ...
                {scenario.horizon && <div className="card-prop stackable">
                  <div className="card-prop-name">Horizon</div>
                  <div className="card-prop-value">{scenario.horizon}</div>
                </div> }
                {scenario.plausibility && <div className="card-prop stackable">
                  <div className="card-prop-name">Plausibility</div>
                  <div className="card-prop-value">{scenario.plausibility}</div>
                </div> }
                ...
                {scenario.signals && <div className="card-prop">
                  <div className="card-prop-name">Signals/Driving Forces</div>
                  <div className="card-prop-value">{(scenario.signals || []).join(', ')}</div>
                </div> }
                {scenario.threats && <div className="card-prop">
                  <div className="card-prop-name">Threats</div>
                  <div className="card-prop-value">{(scenario.threats || []).join(', ')}</div>
                </div> }
                {scenario.opportunities && <div className="card-prop">
                  <div className="card-prop-name">Opportunities</div>
                  <div className="card-prop-value">{(scenario.opportunities || []).join(', ')}</div>
                </div> }
              </div>
            </Card>
          })}
