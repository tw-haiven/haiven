...
const SceneCardCover = ({ scenario }) => {
  ...
  let imageSrc = scenario.visual_prompt
    ? "/api/storyboard-img?prompt=" +
      encodeURIComponent(visualPromptOverride || scenario.visual_prompt)
    : "/loadingicon.gif";

  return (
    <>
      {editMode && (
        <div className="cover-visual-prompt">
          ...
          >
            {visualPromptOverride || scenario.visual_prompt}
          </div>
...
const Home = () => {
  ...
  const onClickTestStrategies = () => {
    ...
    const scenariosParams = subprompts.map(
      (s) =>
        "scenarios=" +
        encodeURIComponent(
          JSON.stringify({
  ...
  const onSubmitPrompt = (value, event) => {
    ...
    const uri =
      "/api/create-storyboard?input=" +
      encodeURIComponent(prompt) +
      "&num_scenes=" +
      encodeURIComponent(numOfScenarios) +
      "&image_style=" +
      encodeURIComponent(illustrationStyle);
    ...
    <>
      ...
      <div id="canvas">
        <div id="prompt-center">
          ...
          <br />
          <br />
          <div>Scenario and story description (multiple lines allowed):</div>
          <Input.TextArea
            autoSize={{ minRows: 1 }}
            placeholder="Describe the story or future scenario. You can break it into scenes and label each scene with a title, eg 'Scene 1: The beginning...'"
            value={prompt}
            onChange={(e, v) => {
          ...
          &nbsp;&nbsp; Generate at least{" "}
          <Select
            defaultValue={"as many scenes as needed"}
            onChange={handleSelectChange}
            style={{ width: 220 }}
            disabled={isLoading}
            options={[
              {
                value: "as many scenes as described in to the prompt",
                label: "as many scenes as needed",
              },
          ...
          &nbsp;&nbsp; Illustration style:{" "}
          <Select
            defaultValue={"graphicart"}
            onChange={handleIllustrationStyleChange}
            style={{ width: 200 }}
            disabled={isLoading}
            options={[
              {
          ...
          {selections.length > 0 && (
            <SelectedItemsMenu
              ...
              items={scenarios}
              onClickTestStrategies={onClickTestStrategies}
            />
          )}
        </div>
        {/* END PROMPT CENTER */}
          ...
          {scenarios?.map((scenario, i) => {
            return (
              <Card
                key={i}
                className="scenario"
                cover={<SceneCardCover scenario={scenario} />}
                actions={[
                  <input
                    key={"cb" + i}
                    type="checkbox"
