'use client';
import { ProChat, ProChatProvider, useProChat, Avatar } from '@ant-design/pro-chat';
...
const Home = () => {
  ...
  const PromptChat = () => {
    ...
    function handlePromptChange(value) {
      const selectedPrompt = prompts.find(prompt => prompt.identifier === value);
    ...
    useEffect(() => {
      fetch('/api/prompts', {
        ...
        response.json().then(data => {
          const formattedPrompts = data.map(item => ({ ...item, value: item.identifier, label: item.title }));
      ...
      <div>
        <div id="prompt-center">
          <h2>Prompting Center</h2>
            What do you want to do? <Select onChange={handlePromptChange} style={{ width: 300 }}
              options={prompts}>
            </Select>
            <div>
              {selectedPrompt && <div>
                  <p><b>Description: </b>{selectedPrompt.help_prompt_description}</p>
                  <p><b>User input: </b>{selectedPrompt.help_user_input}</p>
                  <p><b>Sample input: </b>{selectedPrompt.help_sample_input}</p>
                  </div>}
            </div>
            <br/><br/>
        ...
        {showComponent && (
          <ProChat
            ...
            locale="en-US"
            helloMessage={<div>{'Welcome! To get started, choose a prompt from the dropdown and give me your input.'}</div>}
