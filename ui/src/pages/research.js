    ...
    <div className="selected-items-menu">
      ...
      <Space wrap>
        <Button type="primary" onClick={onClickBrainstormStrategies}>
          Read these articles to answer the research query
        </Button>
      </Space>
    </div>
  );
      ...
      <div id="canvas">
        <div id="prompt-center">
          ...
          <Space>
            ...
            <Search
              ref={promptRef}
              placeholder="enter a research question or prompt"
              onSearch={onSubmitPrompt}
              style={{ width: 700, color: "white" }}
              disabled={isLoading}
              value={prompt}
              onChange={(e, v) => {
          ...
          {scenarios.map((scenario, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div>
                    <b>{scenario.date && scenario.date + ":"}</b>{" "}
                    {scenario.answer ? scenario.answer : scenario.snippet}
                  </div>
                  <br />
                  <div>
                    <b>Source:</b>{" "}
                    <a target="_blank" href={scenario.link}>
                      {scenario.link}
                    </a>
                  </div>
                </div>
