...
const Home = () => {
  ...
  const onExplore = (id) => {
    setDrawerTitle("Explore concept: " + scenarios.concepts[id].title + ": " + scenarios.concepts[id].tagline)
    setChatContext({id: id, originalPrompt: prompt, type: 'concept', summary: scenarios.concepts[id].pitch, ...scenarios.concepts[id]})
    setDrawerOpen(true);
  }
  ...
  const onGenerateVariations = (id) => {
    ...
    // });
    const scenario = scenarios.concepts[id];
    const url = "/concepts?strategic_prompt="+encodeURIComponent("Explore variations of the concept \"" + scenario.tagline + "\": " + scenario.pitch);
    window.open(url, '_blank', 'noreferrer');
  }

  const onClickCreateStoryboard = () => {
    const scenario = scenarios.concepts[selections[0]];
    const url = "/storyboard?prompt=" + encodeURIComponent(prompt + ":\n" + scenario.title+": "+scenario.tagline + "\n" + scenario.pitch)
    window.open(url, '_blank', 'noreferrer');
  }
  ...
  const onGenerateStoryboard = (id) => {
    const scenario = scenarios.concepts[id];
    const url = "/storyboard?prompt=" + encodeURIComponent(scenario.title+": "+scenario.tagline + "\n" + scenario.pitch)
    window.open(url, '_blank', 'noreferrer');
  }
  ...
  const onSubmitPrompt = (value, event) => {
    ...
    const uri = '/api/concepts?input=' + encodeURIComponent(value)
      + '&subprompts=' + encodeURIComponent(JSON.stringify(subprompts))
      + '&num_concepts=' + encodeURIComponent(numOfScenarios)
      ...
      <div id="canvas">
        <div id="prompt-center">
          ...
          <div style={{ display: 'inline' }}>
            ...
            <Search placeholder="enter a prompt (eg How might we...?) to generate concepts"
          ...
          <Select defaultValue={'5'} onChange={handleSelectChange} style={{ width: 150 }} disabled={isLoading}
            options={[
          ...
          {subprompts.map((x,i) => {
            return (<div key={'subprompt-'+i} style={{ marginTop: 5 }}>
              <Select key={'select-'+x} defaultValue={'variations'} value={x.type} onChange={handleSelectChange} style={{ width: 180 }} disabled={isLoading}
                options={[
                  { value: 'variations', label: 'Generate variations of' },
                ]}>
              </Select>
              <Input  key={'input-'+x} style={{ width: 500, marginLeft: 10 }} placeholder="describe a concept you want variations of" value={x.prompt} onChange={onSubpromptChanged(i)} disabled={isLoading}/>&nbsp;
          ...
          <br /><br />
          {selections.length > 0 && <SelectedItemsMenu selections={selections} items={scenarios} onClickCreateStoryboard={onClickCreateStoryboard}/>}
        </div>
        {/* END PROMPT CENTER */}
            ...
            {scenarios?.questions?.map((scenario, i) => {
              return <div key={i} className="card-prop stackable" style={{marginBottom: 20, width: 200}}>
                <div className="card-prop-value">
                  {i+1}. <b>{scenario.question}</b><br/>
                  <small>
                    <a href={'/concepts?strategic_prompt='+encodeURIComponent(scenario.question)} target="_blank">
          ...
          {scenarios?.concepts?.map((scenario, i) => {
            return <Card key={i} className="scenario" title={
              ...
              <div className="card-prop stackable" style={{marginTop: 10}}>
                ...
                <div className="card-prop-value"><b>{scenario.pitch}</b></div>
              </div>
              {scenario.hypothesis && <div className="card-prop stackable" style={{marginTop: 10}}>
                <div className="card-prop-name">Hypothesis</div>
                <div className="card-prop-value">{scenario.hypothesis}</div>
              </div> }
              {/* <div className="card-prop stackable" style={{marginTop: 10}}>
