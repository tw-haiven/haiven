...
(Written with the help of cursor agent using claude-4-sonnet)
...
### Decision
Move MCP server to separate repository after initial development stabilizes.
...
### Implementation
- MCP server is architecturally independent from main Haiven application
- Uses Python with MCP protocol library and HTTP client
