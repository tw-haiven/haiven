...
class TestTokenUsageIntegration(unittest.TestCase):
    ...
    def test_stream_text_chat_token_usage_format(self):
        ...
        with patch.object(self.api, 'log_run'):
            ...
            self.assertTrue(token_usage_found, "Token usage SSE event should be present")
    ...
    def test_stream_json_chat_token_usage_format(self):
        ...
        self.mock_chat_manager.json_chat.return_value = ("session-456", mock_chat_session)
    ...
    def test_token_usage_data_structure_consistency(self):
        ...
        self.mock_chat_manager.streaming_chat.return_value = ("session-1", mock_streaming_chat)
        ...
        with patch.object(self.api, 'log_run'):
            ...
            text_result = self.api.stream_text_chat(
                prompt="Test", chat_category="chat", chat_session_key_value="session-1",
                model_config=self.mock_model_config
            )
            json_result = self.api.stream_json_chat(
                prompt="Test", chat_category="creative-matrix", chat_session_key_value="session-2",
                model_config=self.mock_model_config
            )
            ...
            if text_token_usage and json_token_usage:
                self.assertEqual(text_token_usage["total_tokens"], json_token_usage["total_tokens"])
