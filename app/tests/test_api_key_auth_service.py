...
class TestApiKeyAuthService:
    ...
    def test_authenticate_with_api_key_for_mcp_only(self):
        """Test that authentication for MCP endpoints only works correctly."""
        config = MagicMock()
        repository = MagicMock()
        service = ApiKeyAuthService(config, repository)

        # Mock the authenticate_with_api_key method
        with patch.object(service, "authenticate_with_api_key") as mock_auth:
            # Set up the mock to return a valid user
            ...
            result = asyncio.run(
                service.authenticate_with_api_key_for_mcp_only(request)
            )
